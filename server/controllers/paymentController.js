...
const payment = async (req, res) => {
    ...
    if(token == "TEST_TOKEN") {
        ...
        console.log("success: bypass payment gateway");
        status = "success";
    } else {
    ...
    User.find({ user_token: user.user_id }, async function (err, docs) {
        ...
        if (docs.length !== 0) {
            var Details = {
                ...
                name: token.billing_name? token.billing_name: 'name',
                pass: key,
                price: product.price,
                address1: token.shipping_address_line1 ? token.shipping_address_line1 : 'address',
                city: token.shipping_address_city? token.shipping_address_city : 'city',
                zip: token.shipping_address_zip? token.shipping_address_zip : 'zip',
            };
